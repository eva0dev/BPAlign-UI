...
export default function DisagreementTab() {
  ...
  const disagreementLevels = {
    ...
    medium: cases.filter(
      (c) => getDisagreement(c) >= 0.2 && getDisagreement(c) < 0.5
    ).length,
    high: cases.filter((c) => getDisagreement(c) >= 0.5).length,
  };

  // --- Histograms ---
// Define bucket structures with subcategories for disagreement intensity
const bpBuckets = {
...
// Helper to categorize disagreement level
function getDisagreementCount(c) {
  const votes = Object.values(c.predictions);
  const ones = votes.filter((v) => v === 1).length;
  const zeros = votes.length - ones;
  return Math.min(ones, zeros); // number of disagreeing models
}

function getDisagreementLevel(count) {
  if (count === 1) return "mild";       // 1 model disagreed
  if (count <= 2) return "moderate";    // 2 models disagreed
  return "severe";                      // 4+ models disagreed
}
...
cases.forEach((c) => {
  ...
  const pieData = {
    ...
    datasets: [
      {
        data: [
          disagreementLevels.low,
          disagreementLevels.medium,
          disagreementLevels.high,
        ],
...
const options = {
  ...
  plugins: {
    legend: { position: "top" },
    title: { display: true, text: "Disagreement Histogram" },
  },
  scales: {
    x: { stacked: true },
    y: { stacked: true },
  ...
  return (
    <Box>
      ...
      <Typography>
        Average Disagreement Score: {(avgDisagreement * 100).toFixed(1)}%
      </Typography>
